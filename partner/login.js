...
document.getElementById("form-id").addEventListener("submit", async (e) => {
  e.preventDefault();
  clearError("error-id");
  const idNumber = document.getElementById("username").value.trim();
  ...
  try {
    ...
    const { data, error } = await supabaseClient.functions.invoke("check-user", {
      body: { action: "check_user", username: idNumber }
    });

    if (error || data?.error) {
      throw new Error(data?.error || "Username not found or access denied.");
    }
    ...
    sessionStorage.setItem("temp_username", idNumber);
    showStep(2);
    ...
    showError("error-id", error.message, "username");
  } finally {
...
setupPinInputs("login-pin-group", async (pin, container) => {
  ...
  clearError("error-pin");
  const idNumber = sessionStorage.getItem("temp_username");
  ...
  try {
    ...
    if (authError) {
      ...
      const { data: failData, error: failError } = await supabaseClient.functions.invoke("check-user", {
        body: { action: "auth_failed", username: idNumber }
      });
      ...
      if (failData.isLocked) {
        window.goBackToId();
        showError("error-id", "Account locked for 15 minutes due to multiple failed attempts.", "username");
        return;
      } else {
    ...
    const { data: successData, error: successError } = await supabaseClient.functions.invoke("check-user", {
      body: { action: "auth_success", username: idNumber, partner_id: userId }
    });
...
document.getElementById("form-change-pin").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const userId = authData.user.id;
    const { data: updateData, error: functionError } = await supabaseClient.functions.invoke("check-user", {
      body: { action: "pin_changed", partner_id: userId }
    });
